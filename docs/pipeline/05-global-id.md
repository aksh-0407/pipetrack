...
*injective* (one id per detection per frame → collisions impossible):
...
- **`emit_velocity_gate` (A3) — the teleport fix (40-CONFIRMED).** Walk each id's emitted ground track
  ...
  teleports **367 → 0**, worst jump **2224 → 11.9 m/s**, distinct-id count and agreement **unchanged**,
...
- Think of each track fragment as needing to be "covered" by a path through time; each possible bridge
  (fragment A's end → fragment B's start) has a **cost** = `w_temporal·gap + w_spatial·dist +
...
- **Re-entry leans on weak cues** — with colour dead and pose-shape slow, a re-entering player is matched
  mostly on kinematics, which fails after long occlusions → a fresh id.
...
- **ISSUE-4 — covariance-R is ON in production** (`use_measurement_covariance`), so R is not distance-blind.
  Whether it *measurably helps* is **inconclusive** (OFF-vs-ON: teleports 258→269, agreement noise-level) —
...
|---|---|---|---|
| #1 distance/uncertainty R | ✅ **ENABLED (in prod)** | `use_measurement_covariance: true` (`r_floor 0.15`, `r_ceiling 0.8`), fed by 03 `emit_ground_cov` | OFF-vs-ON (8_init): off → teleports 258→269 (+11), agreement −0.0003 (noise-level). **Marginal / inconclusive on this set — keep/disable is a human decision, not auto-verdicted.** Also = decide-in-3D *measurement* consumption. |
| #2 adaptive lost-window | ✅ **ENABLED (in prod)** | `adaptive_lost_window: true` (`lost_window_max 90`); re-entry gates on | OFF-vs-ON (8_init): off → teleports 258→266 (+8), +1 distinct id, agreement −0.0001 (noise-level). **Marginal — decision deferred to human review.** |
...
| A3 emitted velocity-gate | ✅ **BUILT, 40-conf** (off in prod, on in candidate stack) | `emit_velocity_gate` | teleports 367→0, no ids lost — recommend enable |
...
| `emit_kalman_posterior` | ⚠️ **ON but INEFFECTIVE** | `emit_kalman_posterior: true` | active (differs 8/8) yet teleports persist — [known-bugs BUG-1](known-bugs.md) |
...
covariance — see [`wip/to_do.md`](../../wip/to_do.md).
