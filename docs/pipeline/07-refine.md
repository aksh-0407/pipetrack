...
- **exactly 1 reliable view** → place the joint on that camera's back-projection ray at the canonical
  bone length from its already-placed parent (single-view bone-length lift);
- **0 reliable views** → left NaN for temporal/skeletal-prior fill downstream.
...
fps: 50.0
root_cutoff_hz: 3.0       # lower → steadier hips
limb_cutoff_hz: 6.0       # higher → more responsive limbs
filter_order: 4
face_cutoff_hz: 1.5       # face bones: near-rigid, smooth very hard
foot_cutoff_hz: 2.5       # ankle→toe/heel: heavy (barely articulates)
...
|---|---|---|
| **Full refine pipeline** (bones + smoothing + clamp + relift) | Physically valid skeletons; bone-length CV → ~0; hip jitter reduced | ✅ ACCEPTED, enabled |
