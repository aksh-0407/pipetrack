...
- **Hungarian** (again — the optimal one-to-one matcher from [02](02-tracking.md)): here it matches
  *players → role slots* at lowest total geometric cost, so each role gets exactly one player and vice
  versa.
...
- **v1 epoch solver — ACCEPTED** (fixes-log W5-ROLES): core-role coverage 24/32 → 29/32, both umpires
