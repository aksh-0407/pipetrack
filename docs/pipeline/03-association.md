...
Get it right → one player, one identity everywhere. Get it wrong two ways:
...
```
   ...
   nearly identical from opposite sides → easy to confuse
```
...
angle* around a subject give big parallax → stable geometry. Two cameras looking from **opposite
sides** (≈180°) are **near-collinear** through the subject → **low parallax**: the epipolar lines
...
- **`support`** (0→1) scales the whole sum by how many frames the two tracklets actually co-observe —
...
**The cap fix — `graph_llr_positive_cap: 1.5 → 3.5` (the one production change this session).**
...
couldn't make up the gap, and `support < 1` dragged the total under the 2.0 threshold → **the same
central player was split into two ids** (facing-pair under-merge):

```
true facing-pair match, support ≈ 0.85, cap 1.5:
   0.85 · ( min(ground,1.5)=1.5 + 0.3 + 0.1 + 0.2 ) ≈ 1.79  <  2.0   → NO MERGE  (split id)

same match, cap 3.5:
   0.85 · ( min(ground,3.5)=3.4 + 0.3 + 0.1 + 0.2 ) ≈ 3.40  >  2.0   → MERGE ✓
```
...
people. **40-delivery result:** agreement 0.853 → **0.883**, central-player under-merge 11% → **6%**,
colocated ghost pairs 5 → **0**, same-camera collisions **0**. (Full walk-through in
...
|---|---|---|---|
| cap fix (1.5→3.5) | ✅ **ENABLED — ACCEPTED** | `graph_llr_positive_cap: 3.5` | 40-conf: agreement 0.853→0.883, under-merge 11→6%, coloc 5→0 |
| #1 pose-shape → primary | ✅ **ENABLED** | `graph_shape_enabled: true` (F11 corroboration round) | **OFF-vs-ON (8_init): INERT** (off = byte-identical). Harmless; likely active on 40-set hard clips — needs 40-hard A/B |
...
| (feeds 05) covariance emit | ✅ **ENABLED** | `emit_ground_cov: true` → 05 `use_measurement_covariance` | see 05 |
...
- **ID-5 (★★) Single-linkage can't split → permanent chimeras.** 10–32% of ≥3-view clusters are
